...
passport.use('google', 
    new GoogleStrategy({
        ...
        try {
            const userExist = await usersModel.findOne({ email: profile.emails[0]?.value }); //Google me devuelve muchos mails o 1 solo, depende la persona. En un array, entonces si o si 0 para que me de el primero
            if(userExist){
